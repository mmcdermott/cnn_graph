...
class base_model(object):
    ...
    def predict_components(self, components_data, labels=None, sess=None):
        loss = 0
        ...
        for begin in range(0, size, self.batch_size):
            ...
            feed_dict = {self.ph_labels: batch_labels, self.ph_dropout: 1}
        ...
        if labels is not None:
            return predictions.astype(int), loss * self.batch_size / size
        else:
    ...
    def class_probabilities(self, data, sess=None):
        size = data.shape[0]
    ...
    def fit_components(self, train_datas, train_labels, val_datas, val_labels):
        t_process, t_wall = time.process_time(), time.time()
        ...
        num_samples = train_datas[0].shape[0]
        num_steps = int(self.num_epochs * num_samples / self.batch_size)
        for step in range(1, num_steps+1):

            # Be sure to have used all the samples before using one a second time.
            if len(indices) < self.batch_size:
                indices.extend(np.random.permutation(num_samples))
            idx = [indices.popleft() for i in range(self.batch_size)]

            batch_labels = train_labels[idx]
            component_batches = [train_data[idx,:] for train_data in train_datas]

            feed_dict = {self.ph_labels: batch_labels, self.ph_dropout: self.dropout}
            for ph_data, cb in zip(self.ph_datas, component_batches[:-1]):
                feed_dict[ph_data] = cb if type(cb) is np.ndarray else cb.toarray()
            feed_dict[self.ph_disconnected_nodes] = component_batches[-1]
    ...
    def build_component_graph(self, M_0s, num_disconnected_nodes):
        """Build the computational graph of the model."""

        self.graph = tf.Graph()
        self.ph_datas, self.graph_components_out = [], []
        with self.graph.as_default():
            with tf.name_scope('inputs'):
                self.ph_dropout = tf.placeholder(tf.float32, (), 'dropout')
                self.ph_labels = tf.placeholder(tf.int32, (self.batch_size), 'labels')
                self.ph_disconnected_nodes = tf.placeholder(tf.float32, (self.batch_size,
                    num_disconnected_nodes), 'isolated')
            for component, M_0 in enumerate(M_0s):
                with tf.name_scope('component_%d' % component):
                    # Inputs.
                    with tf.name_scope('inputs'):
                        ph_data = tf.placeholder(tf.float32, (self.batch_size, M_0), 'data')
                        self.ph_datas.append(ph_data)

                    op_final_graph_out = self.infer_component(self.ph_datas[component], self.ph_dropout,
                        component)
    ...
    def infer_component(self, data, dropout, component):
        features = self._infer_component(data, dropout, component)
        return features

    def fully_connected_layers(self, graph_components_out, dropout):
        logits = self._fully_connected_layers(graph_components_out + [self.ph_disconnected_nodes], dropout)
        return logits
...
class cgcnn_components(base_model):
    """
    Graph CNN which uses the Chebyshev approximation.

    The following are hyper-parameters of graph convolutional layers.
    They are lists of lists, each list corresponding to one graph component to be analyzed, each of length
    equal to the number of gconv layers for that graph.
    ...
    """
    def __init__(self, Ls, Fs, Ks, ps, M, num_disconnected_nodes, filter='chebyshev5', brelu='b1relu',
        ...
        assert len(Ls) == len(Fs) == len(Ks) == len(ps)
        self.num_components = len(Ls)
        self.num_disconnected_nodes = num_disconnected_nodes
        for L, F, K, p in zip(Ls, Fs, Ks, ps):
        ...
        M_0s = [L[0].shape[0] for L in Ls]
        self.Ls = []
        for component, p in enumerate(ps):
            self.Ls.append([])
            j = 0
            for pp in p:
                self.Ls[component].append(Ls[component][j])
                j += int(np.log2(pp)) if pp > 1 else 0
        Ls = self.Ls

        # Print information about NN architecture.
        for component, L, F, K, p, M_0 in zip(range(1, self.num_components+1), Ls, Fs, Ks, ps, M_0s):
            Ngconv = len(p)
            Nfc = len(M)
            print('NN architecture for component %d' % component)
        ...
        self.Fs, self.Ks, self.ps, self.M = Fs, Ks, ps, M
        ...
        self.build_component_graph(M_0s, self.num_disconnected_nodes)
    ...
    def fc(self, x, Mout, relu=True, leaky=True):
        """Fully connected layer with Mout features."""
        N, Min = x.get_shape()
        W = self._weight_variable([int(Min), Mout], regularization=True)
        b = self._bias_variable([Mout], regularization=True)
        x = tf.matmul(x, W) + b
        activation = lambda x: tf.maximum(0.2 * x, x) if leaky and relu else tf.nn.relu(x) if relu else x
        return activation(x)

    def _infer_component(self, x, dropout, component):
        ...
        for i, p in enumerate(self.ps[component]):
            with tf.variable_scope('conv{}'.format(i+1), reuse = (component > 0) ):
                ...
                with tf.name_scope('pooling'): x = self.pool(x, p)
        return x

    def _fully_connected_layers(self, xs, dropout):
